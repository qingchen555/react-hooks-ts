...
---2.1 隐藏不必要的元素
---2.2 icon 的位置设计
3-对于共用组件的封装，数据从父组件中传递
4-设计具体的热门推荐相关的数据 P38
查看网易云音乐的api 网站：https://neteasecloudmusicapi.js.org/#/?id=neteasecloudmusicapi

5-设计具体的卡片设计
---5.1 封装一个menu card item component
...
1.2 对于carousel里面包裹的div设置flex无效
因为carousel本身包裹的div 为inline-block

1.3 图片阴影效果

1.4carousel里面包裹的图片为什么width会自动设置为0

## 优化store 多个action部分

## 榜单的设计 P47开始

1.1 获取server数据：promise.all()的使用
数据流理清楚： 在哪里拿的数据，如果传递到页面进行展示，有哪些事件处理数据，如何传到后端服务器
